...
1.	Search — a full search feature in the bottom nav. Keyword search across the story library, plus topic-based timeline search.
2.	Coverage Mapping — inside "How We Know," a toggle between a U.S. state map and a world map showing which regions' papers covered the story.
3.	Below the Radar — a separate section on the home feed surfacing stories that hit the detection threshold (3+ sources) but did not reach the publication threshold (5+ across 2+ editorial perspectives) within 24 hours, where at least one source is Tier 1 (AP or Reuters).
...
The following phrases are locked. Any drift from this language in app copy, marketing, grant applications, or session prompts is a regression that should be corrected.
•	"reconstruct" (not "rebuild") — what Lens does to news stories
•	"facts that survive across sources" (not "verified facts") — Lens's term for confirmed information
•	"There are plenty of platforms" (not "There are other platforms") — competitive framing
...
Logo Direction
Black and white only — applies to the brand marks (the ◎ lens icon and the "lens." wordmark in a clean serif). No navy. No color accents in the brand mark itself. The feel is calm, trustworthy, modern — not flashy, not childish, not corporate. For the full app color palette (which allows functional color for confidence indicators, Coverage Map shading, and a few muted accents), see COLOR PALETTE in Session 1.
Section 2: User Experience
Onboarding Flow
4.	"Who are you?" — User picks reading level: Young Reader (7–10), Teen Reader (11–17), Adult Reader (18+). Adjustable anytime. Controls language complexity, sentence length, vocabulary.
5.	"What do you care about?" — User selects topics from a visual grid: Government & Politics, World News, Science, Space, Technology, Sports, Animals & Nature, Business & Money, Health, Arts & Entertainment, Environment. All on by default, user narrows. Changeable anytime.
6.	"How do you want your news?" — Read It, Hear It, or Both.
7.	"Pick your voice" — User chooses from personality-driven AI narrator voices. Each voice is a distinct character with a name, personality description, and tone.
Launch voice lineup
...
Home Screen: "Lens Daily Brief"
•	Personalized greeting with user's name
•	"Lens Daily Brief" — Audio player card. Compiles top stories into a 5–10 minute audio briefing read by the chosen voice, structured like a real news broadcast without bias. Subtitle on the home card reads "5 stories · ~8 minutes".
...
•	Home — Daily Brief and today's stories.
...
•	Daily Brief builder checks age gate before including a story; gated stories are replaced with the next-highest-ranking eligible story.
...
•	Empty state — if a search returns nothing, show: "No Lens stories match that search yet. Try a broader topic, or browse by category below." Then surface the 11 topic tiles for browsing.
...
Color spec: Covered regions (states or countries with at least one source on this story) are shaded red (#D62828) for visibility, with hover state #E63946 and a green selected stroke. The red color is a deliberate exception to the otherwise restrained Lens palette and exists because small regions and territories disappear at typical map zoom levels when shaded black or gray. This was confirmed in commit ea85d3f.
...
8.	Loaded Language: Words chosen to trigger emotion. Strip and replace with neutral equivalents.
9.	Framing Bias: Which angle does the source lead with? Note without adopting.
10.	Selection Bias: What does this source include/exclude vs. others?
11.	Attribution Bias: Who is quoted? How are they described?
12.	Omission Bias: What is not mentioned? Compare across sources to find gaps.
13.	Narrative Bias: Is the source telling a story with heroes/villains? Strip narrative arc, report events.
14.	Headline vs. Content Mismatch: Go by reported facts, not headline.
System Prompt
...
15.	We do not want "unbiased" sources (they do not exist). We want sources whose biases are known and varied, so the AI can triangulate truth.
16.	Sources are tagged by TYPE and PERSPECTIVE. A source can have multiple tags.
17.	More sources make the engine better, not more complicated.
18.	Every source is vetted and tagged with a reliability rating. Sources known for misinformation are excluded.
19.	Geographic, demographic, and linguistic diversity is a priority. No single region, country, or perspective should be disproportionately represented.
...
20.	Israel: One source (Haaretz) rather than two, balanced by Middle East Eye and Al Jazeera.
21.	China: SCMP kept with ownership caveat and extra scrutiny on sensitive topics.
22.	Ukraine/Russia: Kyiv Independent kept with perspective tag; Meduza added for independent Russian perspective; no Russian state media.
23.	U.S. Regional: 57 papers covering all 50 states and D.C. to eliminate East Coast/Beltway dominance.
24.	Spanish: Planned for V2 (output translation first, then Spanish-language source integration).
25.	Agency press releases: Excluded.
...
Step 7: QC Pipeline — Seven automated checks run before publishing (fact verification, bias filter verification, plagiarism check, reading level verification, age-gate classification, perspective balance, completeness). Stories that pass all checks publish automatically based on confidence tier.
...
Challenge 9: AI Training Data Bias — Regular auditing by reviewers across the political spectrum. Periodic published "bias audits." Continuous prompt and methodology adjustment.
...
Subheadline: For every major news story, Lens pulls coverage from across the media spectrum, identifies the facts that survive across sources, and separates them from the framing. You get clarity. You decide what to think.
...
•	Family and Classroom Sharing — Teacher dashboards, shared feeds with discussion.
...
26.	Paste the prompt. Copy the prompt from this document and paste it into Claude Code. Hit enter.
27.	Watch Claude Code work. It will start writing files, creating folders, and generating code. You will see it narrate what it is doing — things like "Creating src/components/Onboarding.tsx" or "Installing tailwindcss." Let it run. This can take a few minutes. Do not interrupt unless it asks you a question.
28.	Claude Code tells you how to see your work. When done, it usually says "Run npm run dev to see the app locally" or similar. Follow those instructions. Open the URL in your browser (Chrome is best). Open the app on your phone too (same URL if on the same wifi, or deploy to Vercel first).
29.	Check against "what done looks like." Every session has a "what done looks like" description. Compare what you see to that description.
30.	One of three things happens:
...
31.	Save your progress. At the end of every session, tell Claude Code: "Deploy this to Vercel." This pushes your latest version live so you can see it on your phone and so your work is saved.
Tips
...
- COLOR PALETTE: The ◎ lens mark (the circular brand icon) and the "lens." wordmark (lowercase, serif, trailing period) are pure black on warm white (#fafafa). This is non-negotiable for logo treatment. The rest of the app uses a restrained palette: pure black (#000) for headers and primary text, warm white (#fafafa) for backgrounds, medium gray (#666) for secondary text. Functional color is permitted where it earns its place: confidence indicators (high-confidence green dot, developing amber dot), Coverage Map red (per spec above), and one or two muted accent colors for tap-to-define popups and "What We Don't Know" callouts. Navy (#1a1a2e) is not part of the palette and should not appear as a primary or background fill anywhere in the app. Wordmark status: the current canonical wordmark is "lens." (lowercase, serif, trailing period). A new brand mark featuring hand-drawn vintage spectacles paired with "lens." in vintage typewriter type is in development; see Visual Identity Brief. The new mark will replace the current treatment when the illustrator delivers final art. Until then, all references and treatments should use the current "lens." wordmark for consistency.
...
- Visual grid of topic cards, each with an emoji icon and label:
  - 🏛 Government & Politics
  - 🌍 World News
  - 🔬 Science — Scientific research and discoveries: medical breakthroughs, climate research, biology, chemistry, physics, archaeology, paleontology, and how the world works.
  - 🚀 Space — Space exploration, astronomy, NASA missions, planets and moons, telescopes and observatories, and what's happening beyond Earth.
  - 💻 Technology
  - ⚽ Sports
  - 🐾 Animals & Nature
  - 💰 Business & Money
  ...
  - 🎭 Arts & Entertainment
  - 🍃 Environment
- ALL 11 topics are selected by default (highlighted). User taps to DESELECT topics they don't want.
...
🎯 Jackson's task — Onboarding Language Review: After this session, hand Jackson the phone and let him go through both screens. Ask: Does "Who are you?" make sense? Are the topic names clear — would a kid his age know what "Business & Money" covers? Is anything confusing or feel like it's for little kids? Note his feedback for the next session.
...
3. "Lens Daily Brief" card:
- A visually distinct card at the top of the feed
- Shows: "Daily Brief" as title, "5 stories · ~8 minutes" as subtitle
- A large play button
...
🎯 Jackson's task — Home Screen First Impression: After this session, hand Jackson the phone without instructions. Watch what he taps first, what he reads, what he skips. Then ask: What did you think this screen was showing you? What would you tap first? Does anything look cluttered or confusing? Does it feel like a real app?
...
32.	The Anchor — Confident, clear, classic news voice. (American, authoritative but warm, moderate pace)
33.	The Narrator — Warm, storytelling tone. (American, rich and expressive, slightly slower pace)
34.	The Big Sister — Casual, smart, relatable. (American, conversational, natural inflection, slightly faster pace)
35.	Mrs. M — Soothing, crisp, clear, British. (British, warm but precise, measured pace). If voice clone permission has been granted, use Option A. Otherwise, design a voice matching these qualities (Option B).
36.	The Coach — Energetic, encouraging. (American, upbeat, motivating tone, clear enunciation)
...
PART 3: Daily Brief player on home screen
Update the "Lens Daily Brief" card:
...
What "done" looks like: Every story has a working audio player that reads the story in the user's chosen voice. The Daily Brief plays a sequence of top stories. Switching voices in Settings changes which audio plays.
...
9. Test the Daily Brief — does it play the right stories in the right voice?
...
3. "Browse by topic" section (shown when search input is empty):
  ...
  - The 11 topic tiles in a 2-column grid, each with emoji and name
...
- Then show the 11 topic tiles below
...
🎯 Jackson's task — Search Test: After this session, hand Jackson the phone. Ask him to find specific stories he remembers reading. Can he find them easily? Does the date stamp help or confuse him? Is the topic timeline interesting or boring? Note what he says.
...
NEW in v3.1 — This is one of the three features added to MVP v3.1. See Part I, Section 3.2 for full product spec.
...
5. The map:
  - U.S. view: a map of the 50 states + D.C. States with covering sources are shaded red (#D62828) for visibility, with hover state #E63946 and a green selected stroke. States without covering sources are light gray (#e5e7eb) with a thin outline. See Section 3.2 "Color spec" for the reasoning.
  - World view: a world map. Countries with covering sources are shaded red (#D62828, hover #E63946, green selected stroke). Countries without are light gray (#e5e7eb).
...
What "done" looks like: Expand "How We Know" on any story. See the Coverage Map section with the U.S./World toggle. Both views render correctly. Tapping a shaded region shows which source covered the story from there. The summary line below shows the coverage ratio.
If something goes wrong: react-simple-maps sometimes has issues with specific topojson data. If the map does not render, tell Claude Code: "The map is not rendering. Here is what I see:" and describe what's wrong. It may need to try alternative topojson sources.
...
🎯 Jackson's task — Story Topic Selection: Before writing stories, Jackson and Rebecca look at the week's news together and Jackson picks stories he thinks are important and interesting. The goal is diversity across the 11 topics. Jackson has equal say here — these stories represent what Lens is.
How to do it:
 ...
 
{
  ...
  topic: "[one of the 11 Lens topics]",
  topicIcon: "[emoji]",
...
Aim for diversity across the 11 topics. A good initial batch might be:
...
🎯 Jackson's task — Bias Detector (Most Important Job): Read every story at Young Reader level and answer ONE question: Does this story feel like it's trying to make me think a certain way? If yes: What way? Which words or sentences made it feel that way? This is Jackson's superpower — the reason Lens exists.
...
Also update the Daily Brief card on the home screen to reference the actual top stories by source count.
...
37.	Go through onboarding from scratch — is it smooth?
38.	Browse the home feed — do the story cards make you want to tap?
39.	Check Below the Radar — do you understand what it is?
40.	Use Search — can you find a story you remember reading?
41.	Read at least 5 stories cover to cover (all four tabs)
42.	Use Tap to Define on every story
43.	Listen to at least 3 stories using the audio player
44.	Listen to the Daily Brief all the way through
45.	Try switching voices in Settings — does the new voice play?
46.	Try "How We Know" on at least 3 stories — including the Coverage Map. Does it make sense?
47.	Try "Report a Problem" — is it easy to use?
48.	Overall: does this feel like something you'd actually use every morning?
...
🎯 Jackson's task — Below the Radar Feedback: Show Jackson a Below the Radar story. Ask: Do you understand why this story is here and not in the main feed? Does the yellow badge make sense? Would you trust a story with fewer sources? His honest answer matters — if a 9-year-old finds it confusing, we need clearer copy.
...
1. Walk through the entire user flow and check for any broken links, missing content, or visual issues:
  - Fresh user: onboarding → home → browse stories → read a story → all four tabs → How We Know (with Coverage Map) → Go Deeper → Tap to Define → listen to audio → Daily Brief → settings
...
🎯 Jackson's task — The "Show a Friend" Test: Show the app to one friend (with parent permission). Do not explain anything — just hand them the phone. Note: Can they figure out onboarding alone? Do they read a story? Do they try Search? Do they notice Below the Radar? Do they seem interested or bored? What do they say? Report back.
...
Important: Stage 2 is where having a developer (even part-time) would help most. If grant funding comes through during Stage 1, consider hiring someone for this phase. The prompts below assume you are doing it yourself with Claude Code, but they are also a solid spec for handing to a developer.
NEW in v3.1 for Stage 2: Sessions 24, 26, 28, and 29 now include references to Coverage Map generation, Below the Radar detection logic, and Search index maintenance. See callouts within those sessions.
...
3. For each incoming article, store in FEED_ITEMS:
  ...
  - Topic classification (use Claude API to classify into the 11 Lens topics)
...
NEW in v3.1: Clustering service now explicitly tracks clusters that hit detection threshold but not publication threshold, tagging them for Below the Radar evaluation.
Prompt for Claude Code:
CONTEXT: I'm building Lens, a news app that strips bias from reporting. Read /docs/project-bible.md for full context.
...
Sessions 32–33: Daily Brief builder + frontend audio connection
Session 32 prompt:
Build the Daily Brief system.
...
11. Updates the "Lens Daily Brief" card on the home screen with the new briefing
...
1. On the home screen, the "Lens Daily Brief" card should now play the actual Daily Brief audio from Cloudflare R2
...
2. The Daily Brief should remember where the user left off if they pause and come back later.
...
3. Push notification (Daily Brief):
  - "Your Daily Brief is ready" — sent when the user's Daily Brief audio is generated
  - Opt-in only
...
🎯 Jackson's task — Full Week Beta: Use the complete app every day for one week. His checklist: read every story — does it feel unbiased? Test all three reading levels. Use tap-to-define on every story. Listen to the Daily News Show in each voice. Check "What People Think" on every story — are perspectives balanced? Try "How We Know" including the Coverage Map on multiple stories. Check the Below the Radar section daily — do the stories there feel different from Today's Stories? Use Search at least once a day. Find things that are wrong and write them down. Duration: One full week of daily use.
...
2. Make sure the Daily Brief generation works across different time zones
3. Verify the "Report a Problem" submissions arrive at hello@thelens.media correctly
4. Confirm all 11 topic categories are receiving stories (some will be thinner than others — that's OK)
...
New user → register → verify email → onboarding → home feed (Today's Stories + Below the Radar) → read story → all four tabs → How We Know + Coverage Map → Go Deeper → Tap to Define → listen to story → Daily Brief → Search (keyword + topic timeline) → settings → change reading level → verify content updates
Sessions 44–45: Launch
Session 44: Send the launch email (Email 4) to your signup list. Monitor the dashboard for the first 24 hours. Watch for: error rates, cost spikes, QC flag rates, user registration numbers.
...
Why it matters: A real teacher expressing real interest is a quote for the Knight Foundation application.
...
Why it matters: Knowing the competitive landscape strengthens every grant application and investor conversation.
Kid Focus Group
...
Why it matters: Even 3–5 kids' reactions are real user research data.
Pending Decisions
...
Before Stage 1: Vercel (already set up), Domain thelens.media (already registered)
...
49.	Project Bible v3.0 → now Part I above, updated to v3.1
50.	Build Guide v3.0 → now Part II above, updated to v3.1
51.	Consolidated Build Document v1 — technical decisions merged into Part I Section 11 and throughout Part II
52.	V2 and Future Features — updated and included as Part I Section 13
The following documents remain separate and should be uploaded to the project alongside this Master Document:
53.	Bias Filter Engine System Prompt v1
54.	Regional Newspaper Source List — state-by-state with methodology
55.	Source Selection Methodology Note
56.	Knight Foundation Application v8
57.	General Grant/Investor Document v3
58.	Entity Structure Memo v2 (PBC confirmed)
59.	Lens Media PBC Founding Narrative
60.	Funding Needs Analysis & Entity Strategy
61.	Complete Legal Requirements Checklist
62.	Jackson's Build Contribution Plan
63.	Competitive Intelligence Report — February 2026
Version Notes
